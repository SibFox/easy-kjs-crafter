...
namespace EasyKJSCrafter.Common.Logger
{
    public class Logger()
    {
        ...
        {
            FileAccess debug = FileAccess.Open(Global.Paths.LogsPath + "debug.log", FileAccess.ModeFlags.WriteRead);
            debug.SeekEnd();
            FileAccess latest = null;
            if (!toDebug)
            {
                latest = FileAccess.Open(Global.Paths.LogsPath + "latest.log", FileAccess.ModeFlags.WriteRead);
                latest.SeekEnd();
            }
